...
// ─── Trust-score curation — SOURCE OF TRUTH is the committed data file (data/trust-signals.json),
// not code. Edit/extend signals there; the build just loads + resolves them to canonical nodes.
const TRUST = JSON.parse(readFileSync(join(HERE, '..', 'data', 'trust-signals.json'), 'utf8'));
const capDim = (s) => s[0].toUpperCase() + s.slice(1);
const MORAL_FINE = TRUST.moral;
const CURATED_DIMS = Object.entries(TRUST.dimensions).map(([dim, map]) => [dim, map, `gc:${capDim(dim)}Signal`]);
...
const SIGNALS = TRUST.characterSignals;
...
function main() {
  ...
  for (const [name, p, basis] of SIGNALS) {
    const id = pickMax(peopleByKey.get(norm(name))) || nameToId.get(name); if (!id) continue;
