...
// Impact page is shared across ALL `*.churchcore.me` subdomains — "authenticated once, recognized
// everywhere under Impact." It mirrors the AgentSession (the same token already in per-origin
// localStorage); it only sticks on churchcore.me hosts (dev / Vercel default hosts skip it, and
...
//
// HARDENING TODO: a server-set HttpOnly `.churchcore.me` cookie would shrink the XSS surface vs
...
/** A `.churchcore.me` cookie only sticks on churchcore.me hosts (apex or subdomain). */
