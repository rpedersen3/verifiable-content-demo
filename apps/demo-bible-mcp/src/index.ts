...
import { GcpKmsProvider } from '@agenticprimitives/key-custody';
...
// P5 — envelope-encryption key provider for sensitive vault objects. Present only when the KEK + SA are
// configured (GcpKmsProvider wraps/unwraps the per-object DEK under the Cloud-KMS KEK); otherwise the vault
// stores sensitive objects plaintext (the Phase-1 bridge).
const vaultWrapper = (env: { GCP_KMS_ENCRYPT_KEY_NAME?: string; GCP_SERVICE_ACCOUNT_JSON?: string }) =>
  env.GCP_KMS_ENCRYPT_KEY_NAME && env.GCP_SERVICE_ACCOUNT_JSON
    ? new GcpKmsProvider({ cryptoKeyName: env.GCP_KMS_ENCRYPT_KEY_NAME, serviceAccountJson: env.GCP_SERVICE_ACCOUNT_JSON })
    : undefined;
...
// qbsb.impact claim while signing in as their OWN identity — no hardcoding, all read live on-chain.
...
// Who CONTROLS the qbsb.impact name node on-chain (AgentNameRegistry.owner(namehash)). The corpus
// owner must be this address (or the resolved agent). Returns null on no-RPC / timeout.
async function resolveBsbController(env: { A2A_RPC_URL?: string; A2A_NAME_REGISTRY?: string }, agentName = 'qbsb.impact'): Promise<string | null> {
...
// Resolve the agent that controls `qbsb.impact` on-chain (agent-naming). The corpus owner MUST be
// this agent — its canonical CAIP-10 id. Returns null if no RPC or the name is unregistered.
async function resolveBsbOwnerId(env: { A2A_RPC_URL?: string; A2A_CHAIN_ID?: string; A2A_NAME_REGISTRY?: string; A2A_NAME_RESOLVER?: string }, agentName = 'qbsb.impact'): Promise<{ id: string | null; sa: string | null }> {
...
type Env = McpEnv & { DB?: D1Like; ONT?: Relay; ONT_URL?: string; A2A_RELAY?: Relay; RELAY_URL?: string; RELAY_ORIGIN?: string; BSB_TASK_DO?: DurableObjectNamespace; VAULT_OAUTH_SECRET?: string; GCP_KMS_ENCRYPT_KEY_NAME?: string } & A2aEnv;
...
app.post('/tools/resolve', async (c) => {
  ...
  if (d1 && c.env.DB) {
    ...
    if (r) {
      ...
      rowByDescId.set(r.descriptor.id, { corpusRef: d1.corpusRef, corpusRoot: d1.corpusRoot, leafIndex: r.leafIndex, inclusion: () => d1InclusionProof(d1, r.leafIndex), issuerName: 'qbsb.impact' });
    }
  }
...
app.post('/tools/vault_set', async (c) => {
  ...
  await createD1Vault(c.env.DB, vaultWrapper(c.env)).write({ owner: b.owner, resource: b.resource, data: b.data ?? null, classification: b.classification as VaultClassification | undefined });
...
app.post('/tools/vault_get', async (c) => {
  ...
  const r = await gatedVaultRead(c.env.DB, { owner: b.owner, actor, resource: b.resource, fields: b.fields, purpose: b.purpose, audience: VAULT_AUDIENCE, serverId: VAULT_SERVER_ID, resourceUri: VAULT_RESOURCE_URI, wrapper: vaultWrapper(c.env) });
...
app.post('/mcp/vault/read', async (c) => {
  ...
  const r = await gatedVaultRead(c.env.DB, { owner, actor, resource: b.resource, fields: b.fields, purpose: b.purpose, audience: VAULT_AUDIENCE, serverId: VAULT_SERVER_ID, resourceUri: VAULT_RESOURCE_URI, wrapper: vaultWrapper(c.env) });
  if (r.kind === 'not_found') return c.json({ ok: true, object: null });
  if (r.kind === 'denied') {
...
// get_owner_id — resolve who controls qbsb.impact on-chain (the required corpus owner). For the UI + tests.
app.post('/tools/get_owner_id', async (c) => {
  const b = await c.req.json<{ addr?: string; name?: string }>().catch(() => ({}) as { addr?: string; name?: string });
  const name = String(b.name ?? 'qbsb.impact');
...
// claim_service — ownership bound to qbsb.impact (live agent-naming): only its controller may claim
...
app.post('/tools/claim_service', async (c) => {
  ...
  const agentName = String(b.agentName ?? b.issuerAgentId ?? 'qbsb.impact');
  ...
  if (existing) {
    ...
    // wrong claim self-corrects the moment the real qbsb.impact controller connects — no DB surgery).
...
// Inert until A2A_RPC_URL + A2A_AGENT_SA (the claimed qbsb.impact) are configured: the on-chain
